...
//        import org.eclipse.resource.Resource;
//        import org.eclipse.util.IO;
 ...
 * 
 * <p/>
 * The deployer works with a {@link Server} instance which can be either
 ...
 * 
 */
public class Jetty8WarDeployer extends AbstractWarDeployer {
	Logger log = LoggerFactory.getLogger(Jetty8WarDeployer.class);

	/** Jetty system classes */
	// these are loaded by the war parent class-loader
	private static final String[] systemClasses = { "java.", "javax.servlet.",
			"javax.xml.", "org.eclipse." };
	 ...
	 * 
	 * <p/>
	 * To avoid the dependencies on Jetty classes in its signature, this setter
	 * accepts a plain Object that is checked and casted internally.
	 * 
	 * @param server
	 ...
	 */
	protected WarDeployment createDeployment(Bundle bundle, String contextPath)
			throws Exception {
		...
		Jetty8WarDeployment deployment = new Jetty8WarDeployment(
				new JettyContext8Undeployer() {
	...
	protected void startDeployment(WarDeployment deployment) throws Exception {
		Assert.isInstanceOf(Jetty8WarDeployment.class, deployment,
				"Wrong type of deployment used");
		startWebAppContext(((Jetty8WarDeployment) deployment).getWebAppContext());
	}
	 ...
	 * @throws Exception
	 */
	private WebAppContext createJettyWebContext(Bundle bundle,
		...
		// the server is being used to generate the temp folder (so we have to
		// set it)
		...
		// same goes for the context path (add leading "/" -> w/o the context
		// will not work)
		...
		try {
			InputStream in = Jetty8WarDeployer.class.getResourceAsStream("/deployer.properties");
		    properties.load(in);
		} catch (IOException e) {
	...
	private void stopWebAppContext(WebAppContext wac)
		...
		String messageEnding = "context [" + contextPath + "] from server "
				+ getServerInfo();
		...
		try {
			...
			try {
				...
				log.warn(
						"Could not clean unpacked folder for " + messageEnding,
						ex);
			}
		}
	}
	...
	private File unpackBundle(Bundle bundle, WebAppContext wac)
		...
		File tmpFile = File.createTempFile("jetty-"
				+ wac.getContextPath().substring(1), ".osgi");
		tmpFile.delete();
		tmpFile.mkdir();
